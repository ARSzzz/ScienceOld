...
> 设有定义在域 $\mathbb{F}$ 上的两个矩阵 $A_{m\times n}$ 和 $B_{n\times p}$，则两者的乘积（`Product`）$AB$ 为矩阵 $C_{m\times p}$，其中：
> $$C_{ij}=\sum_{k=1}^n A_{ik}B_{kj}$$

当两个矩阵相乘时，只有前者的列数与后者的行数相等时才有意义。同时，矩阵的乘法不符合交换率（`Commutative`），即 $AB=BA$ 不一定成立。

**定理 8：设在域 $\mathbb{F}$ 上有矩阵 $A, B, C$，且下述的乘法均有意义，则有结合律：$A(BC)=(AB)C$**。
...
**定理 9：设有初等行变换 $e$ 和单位矩阵 $I_{m\times m}$，设初等矩阵 $E_{m\times m}=e(I_{m\times m})$。对于所有的矩阵 $A_{m\times n}$，有**：
$$e(A)=EA$$
