...
--选取样本量大于100的疾病
group by substr(Accident_id,5,5) having count( distinct AJBH )>=500
"
...
stable.data <- lapply(cost.interval, Stabilize, least = 10)
names(stable.data) <- names(cost.interval)

# 转换数据
cost.interval <- list()
k <- 1
for (i in 1:length(stable.data)) {
  if (!nrow(stable.data[[i]]) == 0) {
    cost.interval[[k]] <- stable.data[[i]]
    names(cost.interval)[k] <- names(stable.data)[i]
    k = k + 1
  }
}
DM = names(cost.interval)
...
sql.XM <- "
select distinct a.accident_id 事故原因代码,
a.AJBH 案件ID,
b.YYXMDM 项目代码,
b.XMJE/a.SZFY 项目金额比例 
from 

(select distinct (substr(t1.Accident_id,5,5)) accident_id,
t1.AJBH,
t3.SZFY
from  
...
(select a2.AJBH,sum(a1.zfy) szfy from 
WK_GHDJ a1, WK_SGJBQK a2 
where a1.CASENO_YL=a2.AJBH 
and a1.MZZY_KB='2' 
group by a2.AJBH ) t3 
...
(select a2.AJBH,trim(a3.YYXMDM) YYXMDM,sum(JE) XMJE
from WK_GHDJ a1,WK_SGJBQK a2,WK_CFMX a3 
where a1.CASENO_YL=a2.AJBH 
and a1.MZZY_KB='2' 
and a1.GHDJ_CD=a3.GHDJ_CD 
group by a2.AJBH,
trim(a3.YYXMDM) /*项目金额 - 需现场验证*/ ) b

on a.AJBH=b.AJBH 
where a.ajbh in (select CASENO_YL from wk_ghdj where MZZY_KB='2') 
...
WK_SGJBQK a,
WK_XZ_ZR b,
(select AJBH,sum(PFJE) SPF from WK_XZ_ZR group by AJBH having sum(PFJE)>0 ) c
