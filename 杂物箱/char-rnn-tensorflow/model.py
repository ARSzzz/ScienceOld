...
class Model():
    def __init__(self, args, training=True):
        ...
        cells = []
        for _ in range(args.num_layers):  # 两层
            cell = cell_fn(args.rnn_size)  # 神经网络最小的cell
            if training and (
                    args.output_keep_prob < 1.0 or
        ...
        def loop(prev, _):  # pylint: disable=C0111
            prev = tf.matmul(prev, softmax_w) + softmax_b
            prev_symbol = tf.stop_gradient(tf.argmax(prev, 1))
            return tf.nn.embedding_lookup(embedding, prev_symbol)

        # 使用seq2seq来连接神经元的最小单元
    ...
    def sample(self, sess, chars, vocab,
               num=200, prime='The ', sampling_type=1):
        """
        pass
